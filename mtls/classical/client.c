...
// Load client certificate, private key, and CA certificate for server verification
...
int connect_to_server(const char *hostname, int port)
{
    ...
    printf("Connected to mTLS server %s:%d\n", hostname, port);
    return sockfd;
}
...
void interactive_session(SSL *ssl)
{
    ...
    printf("mTLS connection established! Type messages to send to server (Ctrl+C to exit):\n");
    printf("> ");
    fflush(stdout);
