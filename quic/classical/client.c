...
int connect_to_quic_server(SSL *ssl, const char *hostname, int port)
{
    struct sockaddr_in server_addr;
    struct hostent *host;
    BIO_ADDR *peer_addr;
    ...
    // Create BIO_ADDR for QUIC peer address
    peer_addr = BIO_ADDR_new();
    if (!peer_addr)
    {
        fprintf(stderr, "Failed to create BIO_ADDR\n");
        ERR_print_errors_fp(stderr);
        return -1;
    }

    // Set the peer address using the resolved IP and port
    if (BIO_ADDR_rawmake(peer_addr, AF_INET, &server_addr.sin_addr, sizeof(server_addr.sin_addr), port) != 1)
    {
        fprintf(stderr, "Failed to set BIO_ADDR\n");
        ERR_print_errors_fp(stderr);
        BIO_ADDR_free(peer_addr);
        return -1;
    }

    // Set the initial peer address for QUIC
    ...
    printf("QUIC peer address set for %s:%d\n", hostname, port);

    // Free the BIO_ADDR (SSL_set1_initial_peer_addr makes a copy)
    BIO_ADDR_free(peer_addr);
    ...
    printf("Prepared to connect to QUIC server %s:%d\n", hostname, port);
    return 0;
}
