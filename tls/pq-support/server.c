...
// Enhanced function to print post-quantum certificate and key algorithm information
void print_pq_cert_info(SSL_CTX *ctx)
{
...
// Print TLS connection details for post-quantum analysis
...
SSL_CTX *create_ssl_context()
{
    ...
    printf("SSL context created with TLS 1.3 enforcement (required for post-quantum)\n");
    return ctx;
}

// Load post-quantum certificate and private key
int load_pq_certificates(SSL_CTX *ctx)
{
    // Load server certificate
    if (SSL_CTX_use_certificate_file(ctx, "./certs/server-cert.pem", SSL_FILETYPE_PEM) != 1)
    {
        fprintf(stderr, "Failed to load post-quantum server certificate\n");
        ERR_print_errors_fp(stderr);
        return -1;
    }
    printf("Post-quantum server certificate loaded from ./certs/server-cert.pem\n");

    // Load private key
    if (SSL_CTX_use_PrivateKey_file(ctx, "./certs/server-key.pem", SSL_FILETYPE_PEM) != 1)
    {
        fprintf(stderr, "Failed to load post-quantum server private key\n");
        ERR_print_errors_fp(stderr);
        return -1;
    }
    printf("Post-quantum server private key loaded from ./certs/server-key.pem\n");

    // Verify that certificate and private key match
    if (SSL_CTX_check_private_key(ctx) != 1)
    {
        fprintf(stderr, "Post-quantum certificate and private key do not match\n");
        ERR_print_errors_fp(stderr);
        return -1;
    }
    printf("Post-quantum certificate and private key match verified\n");

    // Print certificate and key information
    print_pq_cert_info(ctx);

    return 0;
}

// Set post-quantum specific SSL context options
int configure_pq_context(SSL_CTX *ctx)
{
    ...
    {
        printf("Post-quantum signature algorithms configured\n");
    }
    ...
    {
        printf("Post-quantum key exchange groups configured\n");
    }
...
int create_server_socket(int port)
{
    ...
    printf("Post-quantum TLS server listening on port %d\n", port);
    return sockfd;
}

// Handle client connection with post-quantum TLS
void handle_pq_client(int client_fd, SSL_CTX *ctx)
{
    SSL *ssl;
    ...
    printf("Starting post-quantum TLS handshake...\n");
    int result = SSL_accept(ssl);
    if (result != 1)
    {
        int ssl_error = SSL_get_error(ssl, result);
        fprintf(stderr, "Post-quantum TLS handshake failed (error: %d)\n", ssl_error);
        ERR_print_errors_fp(stderr);
        SSL_free(ssl);
        return;
    }
    printf("Post-quantum TLS handshake successful\n");

    // Print post-quantum TLS connection details
...
int main(int argc, char *argv[])
{
    ...
    printf("=== Post-Quantum TLS 1.3 Echo Server ===\n\n");
    ...
    printf("Post-quantum TLS 1.3 server ready. Press Ctrl+C to stop.\n");
    ...
    while (server_running)
    {
        ...
        handle_pq_client(client_fd, ctx);
        close(client_fd);
