...
// Enhanced function to print post-quantum certificate algorithm information
void print_pq_peer_cert_info(SSL *ssl)
{
    printf("\n=== Server Certificate ===\n");
...
// Initialize OpenSSL and create SSL context for post-quantum support
...
// Configure post-quantum algorithms for client
int configure_pq_client_context(SSL_CTX *ctx)
{
    ...
    {
        printf("Post-quantum signature algorithms configured for client\n");
    }
...
// Load CA certificate for post-quantum server verification
int load_pq_ca_certificate(SSL_CTX *ctx)
{
    if (SSL_CTX_load_verify_locations(ctx, "./certs/ca-cert.pem", NULL) != 1)
    {
        fprintf(stderr, "Failed to load post-quantum CA certificate\n");
        ERR_print_errors_fp(stderr);
        return -1;
    }
    printf("Post-quantum CA certificate loaded from ./certs/ca-cert.pem\n");

    // Enable peer verification
    SSL_CTX_set_verify(ctx, SSL_VERIFY_PEER, NULL);
    printf("Post-quantum server certificate verification enabled\n");

    return 0;
}

// Resolve hostname and create socket connection
int connect_to_pq_server(const char *hostname, int port)
{
    int sockfd;
    ...
    printf("Connected to post-quantum TLS server %s:%d\n", hostname, port);
    return sockfd;
}

// Interactive communication loop for post-quantum TLS
void pq_interactive_session(SSL *ssl)
{
    char send_buffer[BUFFER_SIZE];
    char recv_buffer[BUFFER_SIZE];
    int bytes_sent, bytes_received;

    printf("Post-quantum TLS connection established! Type messages to send to server (Ctrl+C to exit):\n");
    printf("> ");
    fflush(stdout);
...
int main(int argc, char *argv[])
{
    ...
    printf("=== Post-Quantum TLS 1.3 Client ===\n\n");

    // Create SSL context
    ctx = create_pq_ssl_context();
    if (!ctx)
    {
        return 1;
    }
    ...
    if (configure_pq_client_context(ctx) != 0)
    {
        SSL_CTX_free(ctx);
        return 1;
    }
    ...
    // Load CA certificate
    if (load_pq_ca_certificate(ctx) != 0)
    {
        SSL_CTX_free(ctx);
        return 1;
    }
    ...
    // Connect to server
    client_fd = connect_to_pq_server(hostname, port);
    if (client_fd < 0)
    {
        SSL_CTX_free(ctx);
        return 1;
    ...
    print_pq_peer_cert_info(ssl);

    // Start interactive session
    pq_interactive_session(ssl);
